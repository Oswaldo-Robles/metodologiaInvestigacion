(() => {
    ...
    const tbdoyAlumnoSistemas = document.getElementById("tbody-alumnos-sistemas");
    const formInsert = document.getElementById("form-insert");
    const formUpdate = document.getElementById("form-update");

    const dibujarTabla = () => {

        fetch(window.SERVICIOURL + `/apiMetodologia/sistemas.php`)
            .then((response) => response.json())
            .then((data) => {
                console.log(data)

                tbdoyAlumnoSistemas.innerHTML = "";
                data.map(item => {
                    const fila = `
                        <tr data-id="${item.id}"> 
                    ...
                    `
                    tbdoyAlumnoSistemas.innerHTML += fila;
                })

                tbdoyAlumnoSistemas.querySelectorAll(".icono-editar").forEach((iconoEditar, index) => {
    ...
    tbdoyAlumnoSistemas.addEventListener("click", (e) => {
        if (e.target.classList.contains("icono-eliminar")) {
            const fila = e.target.closest("tr");
            const index = [...tbdoyAlumnoSistemas.rows].indexOf(fila);
