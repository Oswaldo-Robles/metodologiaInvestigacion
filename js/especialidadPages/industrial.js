...
(() => {
    ...
    const tbodyIndustrial = document.getElementById("tbody-alumnos-industrial");
        ...
        fetch(window.SERVICIOURL + `/apiMetodologia/industrial.php`)
            ...
            .then((data) => {
                ...
                data.map(item => {
                    ...
                    `
                    tbodyIndustrial.innerHTML += fila;
                })

                tbodyIndustrial.querySelectorAll(".icono-editar").forEach((iconoEditar, index) => {
    ...
    tbodyIndustrial.addEventListener("click", (e) => {
        if (e.target.classList.contains("icono-eliminar")) {
            const fila = e.target.closest("tr");
            const index = [...tbodyIndustrial.rows].indexOf(fila);
